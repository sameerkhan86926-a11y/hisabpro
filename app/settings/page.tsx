...
export default function SettingsPage() {
  ...
  function loadBusinesses() {
    ...
    const oldSaved = localStorage.getItem("hisabpro_business");
    ...
    if (saved) {
      try {
        ...
        console.log("Business data could not be loaded.");
      }
    } else if (oldSaved) {
      try {
        const oldBusiness = JSON.parse(oldSaved);
        ...
        setActiveBusinessId(migratedBusiness.id);

        localStorage.setItem(
          "hisabpro_businesses",
          JSON.stringify([migratedBusiness])
        );

        localStorage.setItem(
          "hisabpro_active_business",
          String(migratedBusiness.id)
        );
      } catch {
        console.log("Old business data could not be migrated.");
      }
    }

    const savedActive = localStorage.getItem("hisabpro_active_business");
    if (savedActive) {
  ...
  function saveBusiness() {
    if (!business.businessName.trim() && !business.ownerName.trim()) {
      setMessage("Please enter Business Name or Owner Name.");
      return;
    }
    ...
    if (editingId !== null) {
      updatedBusinesses = businesses.map((item) =>
        item.id === editingId ? { ...item, ...business } : item
      );
    } else {
    ...
    const activeId = activeBusinessId ?? updatedBusinesses[0]?.id;
    const activeBusiness = updatedBusinesses.find((item) => item.id === activeId);

    if (activeBusiness) {
      localStorage.setItem("hisabpro_business", JSON.stringify(activeBusiness));
    }
  ...
  function deleteBusiness(id: number) {
    const confirmDelete = window.confirm("Delete this business profile?");
    ...
    if (activeBusinessId === id) {
      const nextBusiness = updatedBusinesses[0];
      if (nextBusiness) {
        setActiveBusinessId(nextBusiness.id);
        localStorage.setItem(
          "hisabpro_active_business",
          String(nextBusiness.id)
        );
        localStorage.setItem("hisabpro_business", JSON.stringify(nextBusiness));
      } else {
        setActiveBusinessId(null);
        localStorage.removeItem("hisabpro_active_business");
        localStorage.removeItem("hisabpro_business");
      }
    }
  ...
  function selectBusiness(id: number) {
    ...
    setMessage("Active business changed successfully");
  }
    ...
    <main className="settings-page">
      ...
      <section className="settings-box">
        <div className="settings-title">
          ...
          <div>
            <h2>{editingId !== null ? "Edit Business" : "Add Business"}</h2>
            <p>Save multiple business profiles in one app.</p>
          </div>
        </div>
        ...
        <div className="settings-note">
          ...
          <span>
            You can leave any field blank. Only filled details will appear on the invoice.
          </span>
        </div>
        ...
        <div className="business-form-actions">
          <button className="save-business" onClick={saveBusiness}>
            {editingId !== null ? "Update Business" : "Save Business Details"}
          </button>

          {editingId !== null && (
            <button className="cancel-business" onClick={cancelEdit}>
              Cancel
            </button>
          )}
        </div>
      ...
      <section className="business-list-section">
        <div className="business-list-title">
          <div>
            ...
            <p>Select the business that should appear on invoices.</p>
          </div>
        ...
        {businesses.length === 0 ? (
          <div className="business-empty">
            <div>🏪</div>
            <strong>No business profiles</strong>
            <p>Add your first business above.</p>
          </div>
        ) : (
          <div className="business-list">
            {businesses.map((item) => {
              ...
              return (
                <div
                  className={
                    isActive ? "business-profile active" : "business-profile"
                  }
                  key={item.id}
                >
                  <div className="business-profile-top">
                    <div className="business-profile-icon">🏪</div>
                  ...
                  {item.address && (
                    <p className="business-address">📍 {item.address}</p>
                  )}

                  {item.gstin && (
                    <p className="business-gstin">GSTIN: {item.gstin}</p>
                  )}
